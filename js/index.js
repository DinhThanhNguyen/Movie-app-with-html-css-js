...
import { baseURL, api_key, imageBaseURL, fetchDataFromServer } from './api/apiConfig.js';
...
fetchDataFromServer(`${baseURL}/genre/movie/list?api_key=${api_key}`, function ({ genres }) {
    ...
    fetchDataFromServer(`${baseURL}/movie/popular?api_key=${api_key}&page=1`, heroBanner);
});
...
const heroBanner = function ({ results: movieList }) {
    ...
    for (const [index, movie] of movieList.entries()) {
        ...
        sliderItem.innerHTML = `
            ...
            <div class="banner-content">
                ...
                <p class="gerne">${genreList.asString(genre_ids)}</p>
                <p class="banner-text">${overview}</p>
                <a href="./detail.html" class="btn" onclick="getMovieDetail(${id})">
                    <img src="./img/play_circle.png" width="24" height="24" aria-hidden="true"
                        alt="Play circle">
        ...
        controlItem.innerHTML = `
            <img src="${imageBaseURL}w154${poster_path}" alt="Slider to ${title}" loading="lazy" draggable="false">
        `;
    ...
    for (const { title, path } of homePageSections) {
        fetchDataFromServer(`${baseURL}${path}?api_key=${api_key}&page=1`, createMovieList, title);
    }
};
